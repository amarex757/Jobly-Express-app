...
describe("GET /companies", function () {
  ...
  test("works: filter min/max employees", async function () {
  ...
  test("works: filter min/max and name", async function () {
    const resp = await request(app).get(
      "/companies?minEmployees=2&maxEmployees=3&name=c"
    );
    expect(resp.body).toEqual({
      companies: [
        {
  ...
  test("works and ignores invalid filter value", async function () {
    const resp = await request(app).get("/companies?test=anythingelse&name=c");
    expect(resp.body).toEqual({
      companies: [
        {
          handle: "c1",
